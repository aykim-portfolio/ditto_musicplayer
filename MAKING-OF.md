...
같은 초록의 어두운 끝. 픽셀 서체(둥근모꼴+ Fixedsys, 폴백 Galmuri11),
각진 모서리, 그레인, 하드웨어 디테일.
...
- **픽셀 서체 무게** — 둥근모꼴 `1,577KB` + 가나초콜릿 `483KB` 가 RETRO 첫 진입에
  걸린다. woff2 변환이나 한글 서브셋으로 크게 줄일 수 있지만, 둘 다 파일을
  저장소에 넣어야 해서 방금 지운 셀프호스트 경로가 되살아난다.
  그 교환이 값어치가 있는지 미정 ([32번](#32-둥근모꼴을-되찾았다--그리고-15mb-를-안고-왔다)).
- ~~TWIN 두 줄의 대비를 radius 너머로~~ → **했다.** 원곡에서 그림자를 빼고
  `1px` 하드 보더를 줘서 '둥근 그림자 = 현재 / 각진 테두리 = 과거' 로 갈랐다
  ([33번](#33-twin-의-두-줄이-너무-닮아-있었다)). 아트워크 크기는 아직 같다.
...
## 31. TWIN 이 모바일에서만 안 따라오던 것 — 길이 하나뿐이었다

"양쪽 화면에서 한 줄을 밀면 반대편이 따라오는 모션이 전혀 작동 안 됨.
데스크톱은 잘 되고 모바일만."

### 반대편을 끌고 가는 길이 둘인데, 모바일은 한쪽만 쓴다

데스크톱에는 길이 둘이다.

1. 끄는 동안 — `pointermove` 가 `el.scrollLeft` 를 직접 민다.
2. 손을 떼면 — `endDrag → railScrollTo → done() → onSettle` 이 반대편을 옮긴다.

**둘 다 `scroll` 이벤트가 없어도 돈다.**

모바일에는 그 길이 없다. `pointerdown` 이 터치면 곧바로 빠져나간다 —
그래야 네이티브 스크롤이 살기 때문이고, 그건 25번에서 얻은 결론이다.
그래서 모바일은 실시간 추종도(`scroll → rAF → railSync` 의 TWIN 분기),
안착도(`scroll → nativeScrollTimeout(150ms) → onSettle`) **전부 `scroll` 이벤트
하나에 매달린다.** 그 하나가 늦거나 합쳐지거나 rAF 가 안 돌면 반대편은
아무 데도 안 간다. 한 줄만 혼자 움직이는 게 정확히 그 모양이다.

### 고친 것

터치 동안에는 `scroll` 을 기다리지 않고 직접 따라붙는 루프를 돈다.
`touchstart` 에서 시작해 내 `scrollLeft` 가 더 이상 안 움직일 때까지
(`TOUCH_IDLE_MS = 140`, 관성 포함) 돌다가 스스로 스냅을 복구하고 안착시킨다.

루프는 rAF 로 돌리되 **타이머로 한 번 더 받쳤다.** 이건 새로 만든 규칙이 아니라
이 파일이 이미 `railScrollTo` 의 `glideFallback` 에 적어둔 것과 같은 이유다 —
"rAF 는 화면이 합성되지 않으면 아예 안 돈다." `followStep()` 이 멱등이라
rAF 가 정상이면 인터벌은 할 일이 없다.

### 검증의 한계를 적어둔다

**여기서 이 증상을 정직하게 재현하지 못했다.** 브라우저 창이 숨겨져 있으면
Chrome 이 프레임을 합성하지 않아 `requestAnimationFrame` 이 **아예 안 돌고**
`scroll` 이벤트도 안 뜬다(`rafRan: 0` 으로 확인). 그 환경에서는 rAF 에 기대는
코드가 전부 죽으므로, 처음에 "모바일 재현 성공" 이라고 본 것은 증상이 아니라
계측 환경의 artifact 였다. 데스크톱이 그 환경에서도 안착한 건 순전히
`glideFallback` 타이머 덕이었다.

그래서 확인된 것과 안 된 것을 나눠 적는다.

- **확인됨** — 두 경로의 비대칭은 코드에서 그대로 읽힌다(모바일은 `scroll` 하나에
  전부 걸려 있다). 그리고 타이머 폴백 덕에 이 환경에서도 새 루프가 돌아,
  고치기 전 `A=412 B=0` 이던 것이 고친 뒤 `A=412 B=412` 로 실시간 추종한다.
  스냅도 안착 후 복구된다. 데스크톱 드래그 경로는 이전과 동일(안착 `288/288`),
  콘솔 오류 없음.
- **확인 못 함** — 실제 기기에서 이 증상의 원인이 정말 이것인지.

### 그런데 원인은 코드가 아니었다

폰에서 여전히 안 된다고 해서 그제야 확인해보니, **커밋 9개가 로컬에만 있었다.**
`origin/main` 의 최신은 `5e2d318` 이었고 폰은 계속 그 시점 코드를 보고 있었다 —
TWIN 픽셀 동기화(`bd79b23`)도, 관성 취소(`273ba34`)도, 이 절의 수정(`7bfecda`)도
**하나도 없는 상태**다. "한 줄만 움직인다" 는 게 정확히 그 상태의 증상이다.
push 하니 바로 됐다.

**진단 순서가 틀렸다.** "데스크톱은 되는데 모바일만 안 된다" 는 말을 듣고 곧장
코드 경로부터 팠는데, 그 문장은 *플랫폼이 다르다* 는 뜻이기도 하지만
**보고 있는 빌드가 다르다** 는 뜻이기도 하다. 두 기계가 같은 코드를 보고 있는지
먼저 확인했으면 한 시간이 안 들었다. 앞으로 '한쪽에서만' 이라는 말을 들으면
플랫폼 차이를 뒤지기 전에 **양쪽이 같은 커밋을 보고 있는지부터** 묻는다.

그래서 이 절의 수정이 실제로 필요했는지는 **지금도 모른다.** `bd79b23` ·
`273ba34` 만으로 충분했을 수 있다. 세 커밋이 한꺼번에 배포됐기 때문에 구분이 안 된다.
다만 모바일이 `scroll` 하나에만 매달려 있던 건 사실이고 그 의존은 사라졌으니
그대로 둔다. 되돌려 확인하고 싶으면 커밋은 분리돼 있다.

28번의 관성 스크롤 트릭은 여전히 실기기 미검증이다 — 같은 배포에 섞여 나갔다.

---

## 32. 둥근모꼴을 되찾았다 — 그리고 1.5MB 를 안고 왔다

3번 이래 RETRO 의 디스플레이 서체는 **원래 둥근모꼴+ Fixedsys 를 쓰려던 자리**였다.
공개 CDN 배포본을 못 찾아 같은 계열의 Galmuri11 로 대체해두고, CSS 주석에
"실제 파일을 확보하면 `src` 와 `--font-display` 의 첫 패밀리명만 바꾸면 된다" 고
적어뒀었다. 그 조건이 채워졌다 — 눈누 배포본이 jsdelivr 에 올라와 있다.

바뀐 것은 셋이다.

- `--font-display` 의 첫 후보가 `DungGeunMo` 가 됐다. Galmuri11 은 폴백으로 남는다
  (둥근모꼴에 Bold 가 없어 굵은 자리는 여전히 Galmuri 가 받는다).
- 가나초콜릿체도 같은 방식으로 CDN 을 붙였다. `local()` 을 앞에 두어 시스템 설치본이
  있으면 그걸 먼저 쓰고, 없을 때만 내려받는다. 예전엔 설치 안 된 기계에서 조용히
  Galmuri 로 넘어갔는데, 이제 의도한 서체가 나온다.
- WINAMP 플레이리스트의 곡명(`.wa-name`)에 `-webkit-font-smoothing: none` 을 걸었다.
  픽셀 서체에 안티에일리어싱이 걸리면 계단이 뭉개져서 픽셀 서체를 쓰는 의미가 없다.

`fonts/` 셀프호스트 안내는 두 줄 다 취소선으로 바꿨다 — 이제 받아 넣을 파일이 없다.

### 무게를 적어둔다 — 24번과 정면으로 부딪힌다

실측했다.

| 파일 | 크기 | 형식 |
|---|---|---|
| `DungGeunMo.woff` | **1,577KB** | woff |
| `ghanachoco.woff` | 483KB | woff |
| `Galmuri11.woff2` | 493KB | woff2 (폴백) |

**둥근모꼴 하나가 1.5MB 다.** 24번에서 lucide 405KB 를 5.5KB 로 줄이고
아트워크를 DPR 에 맞춰 고르며 되찾은 무게의 **3.8배**를 한 파일로 다시 얹었다.
RETRO 를 처음 여는 순간 둥근모꼴 1.5MB + 가나초콜릿 483KB 가 걸린다.

`font-display: swap` 이라 화면이 멎지는 않는다 — 폴백으로 먼저 그리고 나중에 바뀐다.
그래서 '느려 보이지 않는' 종류의 무게고, 그게 오히려 놓치기 쉬운 종류다.
24번이 모바일에서 문제를 실측으로 잡아낸 절이라는 걸 생각하면 이건 열어둘 항목이지
넘어갈 항목이 아니다.

줄일 길은 둘이다. **woff2 로 변환**하면(둘 다 woff 다) 보통 30~50% 준다.
**서브셋**은 훨씬 크게 준다 — 이 앱이 픽셀 서체로 찍는 글자는 연도·`ORIGINAL`
같은 라벨·곡 제목 정도라 한글 상용 2,350자만 남겨도 대부분이 날아간다.
다만 둘 다 파일을 직접 만들어 저장소에 넣어야 해서, 방금 지운 `fonts/` 셀프호스트
경로가 되살아난다. **그 교환이 값어치가 있는지는 아직 안 정했다 — 12번에 남긴다.**

확인: 세 CDN 모두 `200`, RETRO 에서 `DungGeunMo` 가 실제로 적용된다
(같은 문자열 폭 `serif 481` 대 `DungGeunMo 460`). 가나초콜릿도 요청 시 정상
로드된다(`502`). 콘솔 오류 없음.

---

## 33. TWIN 의 두 줄이 너무 닮아 있었다

"원곡과 리메이크 아트워크를 감싸는 카드가 너무 똑같다. 극명한 차이를 보고 싶다."

13번에서 연표를 되돌리며 적어둔 문장이 이 화면의 값을 정의한다 —
**두 곡이 계속 화면에 같이 있는 것**. 그런데 같이 있기만 하고 갈라지지는 않았다.
`.era-past` 의 아트 반경이 `4px`, `.era-present` 가 `10px` 이었다.
나란히 놓으면 `4` 는 각진 것으로 안 읽힌다 — **'조금 덜 둥근 것'** 으로 읽힌다.

`--radius-art` 를 `0` 으로 내렸다. `0` 과 `10` 이면 각진 것과 둥근 것이 되어
한눈에 갈린다. `--radius-card` 와 `--radius-art-lg` 도 같이 `0` 으로 맞췄다.

일회성 오버라이드(`.twin-side.original .tw-art { border-radius: 0 }`) 대신
**토큰을 고쳤다.** 2번에서 정한 규칙이 "구분을 색이 아닌 형태에 맡긴다" 이고
`.era-past` / `.era-present` 는 그 규칙을 토큰으로 옮겨둔 것이라, 형태를 더
벌리는 일은 토큰에서 해야 한다. 선택자에서 하면 규칙이 두 군데로 갈라진다.

`.era-past` 는 DOM 전체에서 TWIN 의 원곡 줄에만 붙으므로(`renderTwin` 이 거기서만
입힌다) 다른 화면은 그대로다. 활성 카드의 아웃라인도 `border-radius` 를 따라가
같이 각져진다.

원곡 줄에서 아직 둥근 건 연도 칩(`.tw-year`, `--radius-ctl: 3px`)뿐인데,
리메이크 쪽이 `999px` 알약이라 대비는 이미 충분해서 뒀다.

### radius 만으로는 부족했다 — 물성을 갈랐다

반경을 벌려놓고 보니 두 줄은 여전히 **같은 물건**이었다. 아트워크 크기도(`196px`)
그림자도 여백도 같으니, 모서리만 다른 쌍둥이다.

그래서 원곡에서 **그림자를 빼고 `1px` 하드 보더**를 줬다.

그림자는 '떠 있는 종이' 라는 신호다 — 현재의 카드 UI 문법 그 자체다.
하드 보더는 인쇄물이나 필름 가장자리처럼 '평면에 박혀 있다' 는 신호다.
크기와 여백이 같아도 이 둘은 **서로 다른 물건**으로 읽힌다.
반경 하나로는 안 되던 대비가 여기서 끝난다.

테두리 색은 `color-mix` 로 그 줄의 잉크에 묶었다 — 비활성 `45%`, 활성은 원색.
활성 표시가 아웃라인 하나였는데, 이제 테두리가 같이 진해지면서 두 겹으로 읽힌다.

`box-sizing` 이 `border-box` 라 `1px` 을 더해도 바깥 크기는 `196px` 그대로다.
레일의 가운데 정렬 여백이 그 숫자에 묶여 있어(`padding: … calc((var(--canvas-w)
- 196px) / 2)`) 흔들리면 활성 카드가 중앙에서 밀려난다. 실측으로 확인했다 —
두 줄 다 `172px`(비활성 `scale(.88)` 적용값)로 같다.

확인: 원곡 `radius 0` · `border 1px` · `box-shadow none`, 리메이크 `radius 10px` ·
그림자 유지. 비활성 테두리 `alpha .45`, 활성 원색. 두 줄 폭 동일. 콘솔 오류 없음.

---

## 34. MP3AMP 플레이리스트 — 둥근모꼴은 이미 붙어 있었다

"원곡 모드 MP3 테마의 플레이리스트에서 곡 제목과 아티스트를 둥근모꼴로,
크기는 2px 작게."

앞의 요청을 받고 확인해보니 **서체는 이미 적용돼 있었다.** 32번에서 둥근모꼴을
붙일 때 `.mp3-amp .wa-name` 에 같이 걸어뒀고, 제목과 아티스트는 한 칸에
같이 들어간다(`${era.sub(def)} - ${def.title}`). 실측으로도 확인했다 —
같은 문자열이 `serif 93px` 대 `DungGeunMo 104px` 로 폭이 다르다.

그래서 실제로 바꾼 건 크기 하나다. `.mp3-amp .wa-row` 를 `16px → 14px`.
30번의 짝수 규칙 안에 그대로 떨어진다.

번호(`.wa-idx`)와 연도(`.wa-dur`)는 모노로 남겼다. 자릿수가 세로로 맞아야
목록이 표처럼 읽히는데 픽셀 서체는 그 정렬을 보장하지 않는다.
한 행 안에 서체가 둘 섞이지만, 여기서는 **정렬이 통일보다 값이 크다.**
(`.wa-idx` 는 `flex: 0 0 24px` 로 `10.` 을 받게 돼 있는데, 글자가 작아졌으니
여유는 오히려 늘었다 — 잘림 없음을 확인했다.)

확인: `.wa-name` 서체 `DungGeunMo`, 크기 `14px`, 번호 잘림 없음, 콘솔 오류 없음.
