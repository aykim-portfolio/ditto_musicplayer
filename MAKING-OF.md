...
- **TWIN 두 줄의 대비를 radius 너머로** — 지금은 아트워크 크기·그림자·여백이 같다.
  원곡에서 그림자를 빼고 `1px` 하드 보더를 주면 '둥근 그림자 = 현재 / 각진 테두리
  = 과거' 로 더 갈린다 ([33번](#33-twin-의-두-줄이-너무-닮아-있었다)).
...
**radius 만으로 끝낼 일은 아니다.** 지금 두 줄은 아트워크 크기(`196px`)도 그림자도
여백도 같다. 더 벌리려면 원곡에서 그림자를 빼고 `1px` 하드 보더를 주는 쪽이
효과가 큰데(둥근 그림자 = 현재, 각진 테두리 = 과거), 지시받은 범위가 아니라
손대지 않았다. 12번에 남긴다.

확인: 원곡 `0px` / 리메이크 `10px` 로 실제 적용, `.era-past` 는 TWIN 원곡 줄
하나에만 존재, 콘솔 오류 없음.
