...
## 19. 타임 셔틀 오프셋 수동 보정 도구 및 AI 추정

### 자동 정렬의 한계와 비율 매핑
기존에 `tools/align.html`을 통해 시도했던 크로마 기반 자동 정렬은 템포와 편곡, 보컬이 모두 다른 원곡/리메이크 쌍에서 신뢰도 높은 시간차를 도출하는 데 실패했습니다 (confidence 0.06~0.47).
이를 해결하기 위해 셔틀의 `mapTime()` 로직을 수정했습니다:
1. **비율 기반 매핑**: 절대 초가 아닌 전체 곡 길이 대비 비율로 시간을 계산 (YouTube 전체 곡 모드에서 특히 유용).
2. **수동 보정 폴백**: 수동으로 측정한 offset 값이 있다면 최우선으로 적용.

### 오디오 분석 스크립트 (Onset Detection)
크로마 정렬 대신 **비트 및 음의 시작점(Onset)** 에 기반하여 오디오를 비교하는 방식(`librosa.onset.onset_strength`)을 새롭게 도입했습니다.
- `tools/estimate_offsets.py`
- Cross-Correlation으로 최적의 Offset을 찾아내며, 사용자가 이 스크립트를 로컬에서 돌려 10쌍의 초기 추정치를 얻을 수 있게 구성했습니다.

### 수동 정렬 인터랙티브 도구 (UI)
AI 추정치는 100% 완벽할 수 없으므로, 사람이 귀로 확인하고 보정할 수 있는 전용 도구를 만들었습니다.
- `tools/manual_align.html`
- 두 곡을 나란히 두고, 같은 가사가 나오는 지점에 각각 `마크!` 버튼을 눌러 정확한 Offset을 산출합니다.
- **테스트 셔틀** 기능을 내장해, 버튼 하나로 크로스페이드가 자연스러운지 즉시 귀로 확인할 수 있습니다.
- 결과물은 `js/align.js`에 바로 붙여넣을 수 있는 코드로 출력됩니다.

이로써 타임 셔틀을 탈 때 가사가 되돌아가거나 엉뚱한 곳으로 튀는 품질 문제를 근본적으로 해결할 수 있는 기반이 마련되었습니다.
