...
### 열 개가 다 안 보이게 된 것 — 그대로 뒀다

글자를 키우니 목록 높이가 `195px → 226px` 이 되면서 `max-height: 212px` 를 넘겼다.
열 곡이 한눈에 들어오던 게 **9.6 곡**이 됐다.

`max-height` 를 `228px` 로 올리면 다시 다 들어온다. 그런데 그러면 기기 높이가
`489px → 504px` 가 된다. **지금 TUNER 와 MP3AMP 은 각각 `489px` 와 `489.3px` 로
사실상 같은 높이다** — [15번](#15-mp3amp--몸체는-두고-화면만-갈아끼우기)의 전제가
"기기는 같고 화면만 갈아끼운다" 인데, tweak 으로 넘길 때 기기가 15px 들썩이면
같은 몸체라는 말이 그 자리에서 거짓이 된다.

그래서 높이를 지키고 스크롤을 택했다. 손해가 아니라고 봤다 — **휠이 세로 목록을
넘긴다는 게 MP3AMP 의 존재 이유**인데, 열 개가 다 보이면 휠은 강조 표시만 옮길 뿐이다.
목록이 실제로 넘어가야 실물 클릭휠이 하던 일이 된다. 잘린 열 번째 줄은
"아래에 더 있다"는 신호로도 일한다.

확인: 아트 `132×132`, `border-radius: 0`, 활성 스테이션 중앙 오프셋 `0`.
목록 `font-size 13px`, `10.` 안 눌림, 곡명 잘림 없음, 푸터 넘침 없음.
휠 한 바퀴에 `sel 0 → 9`, `scrollIntoView` 가 10번 줄을 화면 안으로 넣는다
(`scrollTop 12.7 / 최대 14`). 기기 높이는 두 레이아웃 모두 `489px` 유지.
콘솔 오류 없음.
