...
- **'양쪽' 을 연표로 바꿔봤다가 되돌렸다.** 무채색 작업대(`between` 테마) 위에 10쌍의
  연차를 한 축에 얹어 선 길이로 비교하는 화면이었고, 탭 이름 대신 글리프(`■—●`)를 썼다.
  선 길이가 연차에 정확히 비례하는 것(9.91–9.97 px/년)까지 확인했지만,
  **원래의 두 줄 맞물림 쪽이 낫다는 판단으로 되돌렸다.**
  코드는 `git revert` 로 되돌렸으니 필요하면 그 커밋에서 되살릴 수 있다.
  (되돌린 대상: `d726de8` 구현 / `9912a23` 기록)
