...
| |
|:--:|
| <img src="docs/screenshots/10-home-timeline.png" width="240"> |
| 이름 없는 탭. 한 축 위에 10쌍의 연차를 얹어 **선의 길이가 곧 시간**이 된다.<br>왼쪽 끝은 흩어지고 오른쪽 끝은 모인다 — 여러 과거가 하나의 현재로 수렴한다 |
...
`both` 를 정할 때 "양쪽 보기는 어느 한 시대의 편도 들면 안 된다"고 판단해 **앱의 기본 판**에
올렸다. 그런데 이 앱의 기본 판이 곧 **리메이크의 팔레트**다. 그래서 "편을 안 든다"가
결과적으로 "리메이크 편을 든다"가 됐다. 판단은 맞았는데 실행이 틀렸다.

**팔레트만의 문제가 아니었다.** 인터랙션 모델도 파생형이었다:

| 화면 | 모델 |
|---|---|
| 원곡 | 가로 레일, 한 번에 한 곡 |
| 리메이크 | 가로 레일, 한 번에 한 곡 |
| 양쪽(구) | 가로 레일 **두 줄**, 한 번에 한 쌍 |

세 번째가 앞의 둘과 다른 게 "줄이 하나 더 있다"뿐이었다. 색만 바꿔서는 안 고쳐진다.

### 이 화면은 무엇을 위한 자리인가

- **원곡** — 과거를 둘러본다
- **리메이크** — 현재를 둘러본다
- **사이** — **잰다.** 관계 자체가 주인공인 유일한 화면

예전 화면은 한 번에 한 쌍만 보여줘서 `34년` 을 봐도 그게 먼 건지 가까운 건지 알 수 없었다.
비교 대상이 없으니까.

### 답: 한 축 위의 연표

가로 레일을 버리고 **세로 연표**로 갔다. 10쌍을 한 축에 얹으면 **선의 길이가 곧 시간**이다.

```
1981 ■━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━● 2015   34년   청춘
1993 ■━━━━━━━━━━━━━━━━━●                2013   20년   너에게
2004 ■━━━━━━━━━●                        2015   11년   걱정말아요 그대
```

실측하니 10행 전부 **9.91–9.97 px/년**(편차 0.06px, 반올림 수준)이라
길이가 정말로 연차를 인코딩한다. 34년이 11년보다 세 배 길다는 걸 숫자가 아니라 길이로 안다.

정렬은 원곡 연도순이다. 그랬더니 **왼쪽 끝은 흩어지고 오른쪽 끝은 모이는** 모양이 나왔다 —
여러 과거가 하나의 현재(2012–2017 응답하라 시기)로 수렴한다. 의도한 게 아니라 데이터가
가진 성질인데, 정렬을 그렇게 잡으니 저절로 드러났다.

### 세 번째 표면

민트도 짙은 초록도 아닌 **무채색 작업대**(`[data-theme="between"]`)를 새로 뒀다.
두 시대의 유물을 올려놓고 거리를 재는 자리. 모서리도 두 시대의 중간값이고,
축과 수치는 계측기의 글자라 모노를 쓴다.

끝점의 모양이 시대를 말한다 — **왼쪽은 각지고(과거) 오른쪽은 둥글다(현재).**
앱의 모서리 규칙을 그대로 점 두 개에 옮겼다.

### 이름을 없앴다

'양쪽' 은 **개수를 세는 말**이라 무엇을 보여주는지 말하지 않았다.
그렇다고 다른 이름을 붙이기도 애매했다 — 원곡·리메이크는 '곡의 판본'인데
이 탭은 판본이 아니라 둘 사이의 거리라서, 같은 층위의 이름을 붙이면 오히려 거짓말이 된다.

그래서 이름 대신 **글리프**를 넣었다.

```
■———●     각진 점(과거) ─ 선(사이) ─ 둥근 점(현재)
```

이 탭이 여는 화면의 축소판이자, 앱의 모서리 토큰 체계가 그대로 들어간 그림이다.
탭 셋 중 하나만 글자가 아닌 게 어색해 보일 수 있는데, **이 탭만 다른 종류의 것이라
오히려 정직하다.**

읽기 어려움은 두 곳에서 받친다 — `aria-label`·`title` 로 '사이'를 달았고,
바로 아래 목록 캡션이 `사이 BETWEEN · TIMELINE` 으로 이름을 그대로 적는다.

> TWIN(두 줄 맞물림)은 지우지 않고 노출에서만 뺐다. RACK·DIAL 과 같은 처리 —
> `ERA.both.layouts` 에 이름만 되돌리면 살아난다.
