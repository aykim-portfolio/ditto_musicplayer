...
└── tools/
    ...
    ├── estimate_offsets.py  # onset 기반 자동 추정 (참고용)
    └── fetch_catalog.ps1    # iTunes 메타 스냅샷 생성
```

## 알려진 제약 (PRD 4-3)

- **셔틀 전환 시 가사 어긋남:** iTunes 미리듣기는 원곡과 리메이크에서 다른 지점을 발췌하므로, `align.js` 에 수동 오프셋을 채우기 전까지 같은 시간으로 넘어간다 (가사가 되돌아가거나 튀는 증상). `tools/manual_align.html` 로 측정 후 해소 가능.
