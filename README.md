...
| PRD | 구현 |
|---|---|
| 타임 셔틀 토글 (P0) | 플레이어 하단 ORIGINAL ↔ SELECTED 슬라이더. 현재 시간 저장 → 새 음원 seekTo → 크로스페이드 |
| 크로스페이드 | 800ms 볼륨 램프 ([js/player.js](js/player.js)) |
| 버퍼링 안전 락 (PRD 5-2) | 새 음원 `canplay`/`PLAYING` 확인 후에만 기존 음원 페이드아웃 |
| 듀얼 테마 스킨 (P0) | 원곡 모드 = RETRO(베이지/카세트), 리메이크 모드 = MODERN(민트/미니멀) 실시간 전환 |
| 글리치 타임슬립 연출 (PRD 5-1) | 셔틀 시 스캔라인 글리치 오버레이 + 햅틱(지원 기기) |
...
- 두 음원의 구조 차이(인트로 길이, BPM)로 절대 시간 싱크에 오차가 있을 수 있습니다.
